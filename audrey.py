...
def process_data(uploaded_file):
    ...
    if 'Destination' in file_rec.columns:
        # If the column exists, perform the specified operations
        file_rec = file_rec.dropna(subset=['Port of Discharge', 'Destination', 'Port of Loading'], how='all')
        file_rec['Destination_pod'] = file_rec['Destination'].fillna(file_rec['Port of Discharge'])
    else:
        # If the column does not exist, perform alternative operations
        file_rec = file_rec.dropna(subset=['Port of Discharge', 'Port of Loading'], how='all')
        file_rec['Destination_pod'] = file_rec['Port of Discharge']
    ...
    lumpsum = file_rec[file_rec['Charge Code'] == 'Lumpsum'][
        ['port_of_loading2', 'port_of_discharge2', 'left_container', 'Amount', 'Curr.']]
    surcharge = file_rec[file_rec['Charge Code'] == 'MFR'][
        ['port_of_loading2', 'port_of_discharge2', 'left_container', 'Amount']]
    ets = file_rec[file_rec['Charge Code'] == 'ETS'][
        ['port_of_loading2', 'port_of_discharge2', 'left_container', 'Amount']]
    ...
    final = final[
        ['POL', 'POD', 'CONTAINER', 'FREIGHT', 'LINER', 'Currency', 'Surcharge', 'ALL_IN', 'Detention', 'Demurrage']]
